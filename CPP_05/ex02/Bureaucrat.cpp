...
{
	if (form.get_signature() == false)
	{
		std::cout << "<" << this->name_bureaucrat << "> not executes <" << form.getName() <<
				">. The form is not signed." << std::endl;
	}
	else if (value > form.getGrade_signing())
	{
		std::cout << "<" << this->name_bureaucrat << "> not executes <" << form.getName() <<
				  ">. The rating is below the minimum possible." << std::endl;
	}
	else
		std::cout << "<" << this->name_bureaucrat << "> executes <" << form.getName() << ">" << std::endl;
}
